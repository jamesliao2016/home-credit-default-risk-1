...
def load(idx):
    prev = pd.read_feather('./data/prev.agg.feather')
    for fname in [
        ...
        gc.collect()
        prev = prev.merge(pd.read_feather(fname), on='SK_ID_CURR', how='left')

    print(prev.shape)
    test = pd.read_feather('./data/application_test.preprocessed.feather')
    test = test.merge(prev, on='SK_ID_CURR')
    test['TARGET'] = np.nan
    train = pd.read_feather('./data/application_train.preprocessed.feather')
    train = train.merge(prev, on='SK_ID_CURR')
    del prev
...
def train(idx):
    ...
    save_importance(bst, './data/prev.importance.{}.csv'.format(idx))
...
def main():
    ...
    stack = stack.rename(columns={'PRED': 'PREV_STACKED'})

    print(stack.shape)
    print(stack['SK_ID_CURR'].unique().shape)

    stack.to_feather('./data/prev.stack.feather')
